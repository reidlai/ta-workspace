...
export class ModuleLoader {
    ...
    static async loadModules(context: IContext, config: IAppConfig[]): Promise<void> {
        console.log('ModuleLoader: discoverable modules', Object.keys(this.moduleGlob));
        ...
        for (const moduleConfig of config) {
            ...
            if (matchedKey) {
                ...
                try {
                    const module = await this.moduleGlob[matchedKey]() as { default: any, init?: ModuleInit };
                ...
                console.warn(`Module source not found for: ${moduleId} (${moduleConfig.src})`);
            }
        }
    }
}
