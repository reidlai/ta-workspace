...
When(
  "the {string} updates the {string} channel to {string}",
  function (moduleName, channel, value) {
...
Then(
  "the {string} should reflect {string} for {string}",
  function (componentName, value, channel) {
...
Then(
  "other modules subscribed to {string} should receive {string}",
...
When(
  "{string} updates {string} to {int} immediately after",
  function (moduleName, channel, value) {
    mockState.set(channel, value);
  },
);
