<script lang="ts">
  ...
  const stateStore = gadgetRegistry.getStateStore("demo-widget");
  ...
  if (stateStore) {
    ...
    const counterChannel = stateStore.getChannel("demo.counter", 0);

    // Subscribe to changes
    unsubscribe = counterChannel.subscribe((value: number) => {
      count = value;
    });
  }
