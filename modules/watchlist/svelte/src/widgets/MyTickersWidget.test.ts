...
vi.mock('../../../../../apps/sv-appshell/src/lib/components/ui/card', () => ({
    ...
    Header: { render: () => '<header data-testid="card-header"><slot /></header>' },
    Title: { render: () => '<h3 data-testid="card-title"><slot /></h3>' },
    Description: { render: () => '<p data-testid="card-description"><slot /></p>' },
    Content: { render: () => '<div data-testid="card-content"><slot /></div>' }
}), { virtual: true }); // virtual if module doesn't exist
