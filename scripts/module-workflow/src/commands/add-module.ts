...
export function registerAddModuleCommand(program: Command, workspaceRoot: string) {
    program
        .command('add-module')
        ...
        .action(async (options) => {
            ...
            try {
                ...
                if (stacks.hasSvelte) {
                    ...
                    rollback.add(async () => await configUpdater.removeModuleFromRegistry(name));
                }
                ...
                if (stacks.hasTs) {
                    ...
                    rollback.add(async () => await configUpdater.removePackageFromWorkspace(`modules/${name}/ts`));
                }
                if (stacks.hasSvelte) {
                    console.log('Updating pnpm-workspace.yaml (svelte)...');
                    await configUpdater.addPackageToWorkspace(`modules/${name}/svelte`);
                    rollback.add(async () => await configUpdater.removePackageFromWorkspace(`modules/${name}/svelte`));
                }

                if (stacks.hasGo) {
                    console.log('Updating go.work...');
                    await configUpdater.addProjectToGoWork(`modules/${name}/go`);
                    rollback.add(async () => await configUpdater.removeProjectFromGoWork(`modules/${name}/go`));
                }
