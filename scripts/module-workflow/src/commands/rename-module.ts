    ...
    .action(async (options) => {
      ...
      try {
        ...
        if (stacks.hasSvelte) {
          ...
          rollback.add(
            async () =>
              await configUpdater.renameModuleInRegistry(newName, oldName),
          );
        }
        ...
        if (stacks.hasSvelte) {
          ...
          await configUpdater.removePackageFromWorkspace(
            `modules/${oldName}/svelte`,
          );
          await configUpdater.addPackageToWorkspace(
            `modules/${newName}/svelte`,
          );
          rollback.add(async () => {
            await configUpdater.removePackageFromWorkspace(
              `modules/${newName}/svelte`,
        ...
        if (stacks.hasGo) {
          ...
          rollback.add(async () => {
            await configUpdater.removeProjectFromGoWork(
              `modules/${newName}/go`,
            );
        ...
        console.log(
          `Module renamed successfully from '${oldName}' to '${newName}'.`,
        );
      } catch (error) {
